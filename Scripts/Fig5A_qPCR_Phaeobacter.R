...
Fig6A.zoom = ggplot(as.data.frame(qpcr_sum1) %>% filter(56<Time) %>% filter(System != "Control"),
                   aes(x = as.factor(Time), y = mean_c, fill=System, color = System, shape = System)) +
  geom_point(data = qpcr_sum %>% filter(56<Time) %>% filter(System != "Control"),
