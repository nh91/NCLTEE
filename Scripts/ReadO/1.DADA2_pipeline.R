...
pathF <- "demultiplexed/FWD/" # Path to forward reads
pathR <- "demultiplexed/RVD/" #Path to reverse reads

# Forward and reverse fastq filenames have format: SAMPLENAME_R1_001.fastq and SAMPLENAME_R2_001.fastq
fnFs <- sort(list.files(pathF, pattern="_R1_001.fastq.gz", full.names = TRUE))
fnRs <- sort(list.files(pathR, pattern="_R2_001.fastq.gz", full.names = TRUE))
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(242,242),
                     maxN=0, maxEE=c(2,2), truncQ=2,trimLeft = c(17,21),
...
# Make error models
errF <- learnErrors(filtFs, multithread=15, verbose = TRUE, nbases = 1e9)
...
tt.plus = addSpecies(tt, "silva_assignment_sep2021/silva_species_assignment_v138.1.fa.gz", verbose = TRUE, tryRC = TRUE, multithread = TRUE)
